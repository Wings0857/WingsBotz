...
const xteam = 'WingsBotzWa' //Free Trial 3 day
const namabot = 'Wings Botz'

//kontak
const vcard = 'BEGIN:VCARD\n'  //Jangan diganti,Ntar error
            + 'VERSION:3.0\n'  //Jangan diganti,Ntar error
            + 'FN:Wings\n'  // Ganti jadi namamu
            + 'ORG:Pengembang ${WingsBotz};\n'  // Ganti jadi namamu/Botmu
...
memberlimit = 291//Terserah Mo Ganti apa kgk
cr = '*Wings Botz By Wings Store*'
vr = '*Wings Botz Wa

//owner number
const ownerNumber = ["6285708127934@s.whatsapp.net"]  //Ganti Jadi Nomormu
...
if (isLimit(sender)) return reply(ind.limitend(pusname))
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ted = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo: mek
	  reply(ind.wait())
	  owgi = await Wings.downloadAndSaveMediaMessage(ted)
	  tels = body.slice(7)
	  anu = await imgbb("08579d070df9a07cb1c2ee565aece767", owgi)
	  hehe = await getBuffer(`https://videfikri.com/api/textmaker/burneffect/?urlgbr=${anu.display_url}`)
	 akira.sendMessage(from, hehe, image, {quoted:mek})
		...
		if (!isGroupAdmins) return reply(ind.admin())
              if (args[1]=="detik") {var timer = args[0]+"000"
				...
				setTimeout( () => {
					...
					Wings.groupSettingChange (from, GroupSettingChange.messageSend, true);
					reply(close)
				}, timer)
				break
		case 'textdaun':
